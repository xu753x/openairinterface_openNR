...
NR_UE_RRC_INST_t* openair_rrc_top_init_ue_nr(char* rrc_config_path){
  ...
  if(NB_NR_UE_INST > 0){
    ...
    if (get_softmodem_params()->phy_test==1 || get_softmodem_params()->do_ra==1 || get_softmodem_params()->sa == 1) {
	  ...
	  {
	      if (rrc_config_path)
	        sprintf(filename,"%s/ueconfig.raw",rrc_config_path);
	      else
	        sprintf(filename,"ueconfig.raw");
	      fd = fopen(filename,"r");
	      AssertFatal(fd,
	                  "cannot read file %s: errno %d, %s\n",
	                  filename,
		  ...
		  while(!feof(fd))
		  {
			...
			char arch[20] = {0};
			strtok(buffer, ":");
			strcpy(arch, strtok(NULL, ":"));
			printf("buffer value is %s and arch is %s\n",buffer,arch);
			if(memcmp(buffer,"carrierBandwidth",sizeof("carrierBandwidth")) == 0)
			{
				NR_UE_rrc_inst->ueInit.Bandwidth = atoi(arch);
			}
			else if (memcmp(buffer,"scs",sizeof("scs")) == 0)
			{
				NR_UE_rrc_inst->ueInit.subcarrierSpacing = atoi(arch);
			}
			else if (memcmp(buffer,"absoluteFrequencySSB",sizeof("absoluteFrequencySSB")) == 0)
			{
				NR_UE_rrc_inst->ueInit.absoluteFrequencySSB = atoi(arch);
			}
			else if (memcmp(buffer,"band",sizeof("band")) == 0)
			{
				NR_UE_rrc_inst->ueInit.band = atoi(arch);
			}
			else if (memcmp(buffer,"phy_cell_id",sizeof("phy_cell_id")) == 0)
			{
				NR_UE_rrc_inst->ueInit.phy_cell_id = atoi(arch);
			}
			else
			{
				LOG_E(RRC,"NR_RadioBearerConfig decode error\n");
		  ...
		  fclose(fd);
	  	  LOG_I(RRC,"UE Init config Bandwidth %d,scs %d absoluteFrequencySSB %d "
		  	"band %d phy_cell_id %d\n",NR_UE_rrc_inst->ueInit.Bandwidth,NR_UE_rrc_inst->ueInit.subcarrierSpacing,
		  	NR_UE_rrc_inst->ueInit.absoluteFrequencySSB,
		  	NR_UE_rrc_inst->ueInit.band,NR_UE_rrc_inst->ueInit.phy_cell_id);
	  }		
